...
class TasksPath(SqlApiPath):
    ...
    async def get(self):
        ...
        """
        data, headers = await self.get_list()
        return self.json_response(data, headers=headers)
...
class TaskTransactionsPath(SqlApiPath):
    ...
    async def get(self):
        ...
        """
        async with self.db.transaction() as conn:
            data, headers = await self.get_list(conn=conn)
            return self.json_response(data=data, headers=headers)
