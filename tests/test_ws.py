...
async def test_invalid_rpc(cli):
    async with cli.ws_connect('/stream') as ws:
        await ws.send_json(
            dict(method='echo', payload=dict(message='hello'))
        )
        msg = await ws.receive()
        assert msg.type == aiohttp.WSMsgType.TEXT
        data = msg.json()
...
async def test_rpc_echo(cli):
    async with cli.ws_connect('/stream') as ws:
        await ws.send_json(
            dict(id='abc', method='echo', payload=dict(message='hello'))
        )
        msg = await ws.receive()
        assert msg.type == aiohttp.WSMsgType.TEXT
        data = msg.json()
...
async def test_invalid_rpc_method(cli):
    async with cli.ws_connect('/stream') as ws:
        await ws.send_json(
            dict(id='abc', method='wtf', payload=dict(message='hello'))
        )
        msg = await ws.receive()
        assert msg.type == aiohttp.WSMsgType.TEXT
        data = msg.json()
        ...
        assert data['error']
        assert data['error']['message'] == 'Invalid RPC parameters'
        assert data['error']['errors'] == dict(
            method='wtf method not available')
...
async def test_rpc_publish(cli):
    async with cli.ws_connect('/stream') as ws:
        await ws.send_json(
            dict(
                id='abc', method='publish',
                payload=dict(channel='test', data='hello')
            )
        )
        msg = await ws.receive()
        assert msg.type == aiohttp.WSMsgType.TEXT
...
async def test_rpc_subscribe(cli):
    async with cli.ws_connect('/stream') as ws:
        await ws.send_json(
            dict(
                id='abc', method='subscribe',
                payload=dict(channel='test')
            )
        )
        msg = await ws.receive()
        assert msg.type == aiohttp.WSMsgType.TEXT
